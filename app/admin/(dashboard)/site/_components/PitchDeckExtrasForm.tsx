...
export default function PitchDeckExtrasForm({ initial }: { initial: AccountingServiceCard[] }) {
  ...
  async function onSave(e: React.FormEvent) {
    ...
    try {
      await patchSite({ servicePages: { pitchDeck: { serviceCards: items } } });
      setMsg("Saved.");
    } catch (ex) {
  ...
  return (
    <div>
      ...
      <p className="admin-hint" style={{ marginBottom: "1rem" }}>Service cards shown in the grid on the pitch deck &amp; valuation page.</p>
      <form onSubmit={onSave}>
          ...
          <div key={i} style={{ borderBottom: i < items.length - 1 ? "1px solid #F0F0ED" : "none", paddingBottom: "1rem", marginBottom: "1rem" }}>
            ...
            <div className="admin-grid-2">
              <div className="admin-field">
                <label className="admin-label">Icon name (e.g. Presentation, FileText, BarChart3, TrendingUp)</label>
        ...
        <button type="button" className="admin-btn" onClick={() => setItems(p => [...p, { iconName: "Presentation", title: "", desc: "", features: [] }])} style={{ marginBottom: "1rem" }}>+ Add card</button>
